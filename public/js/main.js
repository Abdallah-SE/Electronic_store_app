...
$('a.menu_switch').click(function(evt)
{
    ...
    if($(this).attr('data-menu-status') == 'false') {
        ...
        if(getCookie('menu_opened') == "") {
            setCookie('menu_opened', true, 180, 'mvcapplication.dev');
        }
    } else {
        ...
        deleteCookie('menu_opened', 'mvcapplication.dev');
    }
});
...
(function(){
    ...
    if(null !== userNameField){
        userNameField.addEventListener('blur', function(){
            ...
            req.open('POST', 'https://mvcapplication.dev/users/userExistsAjax');
            req.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            req.onreadystatechange = function(){   
                var createIElement = document.createElement('i');
                if(req.readyState == req.DONE && req.status ==200){
                    if(req.response == 1){
                        createIElement.className = 'fa fa-times error';
                    }else if(req.response == 0){
                        createIElement.className = 'fa fa-check success';
                    }
                    ...
                    userNameField.parentNode.appendChild(createIElement);
                                        for ( var i = 0, ii = iElems.length; i < ii; i++ )
                    {
                        if(iElems[i].nodeName.toLowerCase() == 'i') {
                            iElems[i].parentNode.removeChild(iElems[i]);
